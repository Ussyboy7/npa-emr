...
const InjectionForm: React.FC<InjectionFormProps> = ({
  onSubmit,
  patientName,
  currentNurse,
}) => {
...
const createInitialPatients = (): Patient[] => [
  {
    ...
    injectionNotes:
      "Patient has Type 1 diabetes, requires insulin before meals",
  },
  {
    id: "P002",
    patientId: "P002",
...
const injectionsByPatientId: Record<string, PatientInjectionData> = {
  P001: {
    patientId: "P001",
    ...
    injectionsHistory: [
      {
        ...
        injection: {
          ...
          comment:
            "Patient reports mild burning sensation during administration.",
        },
 ...
 */
const getInjectionIcon = (alerts: string[] = []) => {
...
export default function PatientInjections() {
  ...
  const [selectedPatientInjections, setSelectedPatientInjections] =
    useState<PatientInjectionData | null>(null);
  const [selectedPatientForInjection, setSelectedPatientForInjection] =
  ...
  const priorityOrder: Record<Patient["priority"], number> = {
    Emergency: 0,
    High: 1,
    Medium: 2,
    Low: 3,
  ...
  const stats = {
    ...
    awaitingInjection: patients.filter((p) => p.status === "Awaiting Injection")
      .length,
    injectionComplete: patients.filter((p) => p.status === "Injection Complete")
      .length,
    completed: patients.filter((p) => p.status === "Completed").length,
    emergencyPatients: patients.filter((p) => p.priority === "Emergency")
      .length,
    averageWait: patients.length
      ? Math.round(
          patients.reduce(
            (sum, p) => sum + getMinutesDifference(p.visitTime),
            0
          ) / patients.length
        )
      : 0,
    patientsWithAlerts: patients.filter(
      (p) => p.injectionAlerts && p.injectionAlerts.length > 0
    ).length,
  };
  ...
  const handleCompleteInjection = async (patientId: string) => {
    setIsLoading(true);
    try {
      setPatients((prev) =>
        prev.map((p) =>
  ...
  const handleAddInjection = (data: {
    ...
    console.log(
      "New injection added for patient:",
      selectedPatientForInjection.name,
      data
    );
    handleCompleteInjection(selectedPatientForInjection.id);

    // Update history (mock)
    const patientHistory = injectionsByPatientId[
      selectedPatientForInjection.id
    ] || {
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <CardDescription>
                Patients sent from nursing queue for injections
              </CardDescription>
            </div>
          </div>
      ...
      <CardContent className="p-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                ...
                <SelectItem value="Awaiting Injection">
                  Awaiting Injection
                </SelectItem>
                <SelectItem value="Injection Complete">
                  Injection Complete
                </SelectItem>
        ...
        <div className="grid gap-4">
          {paginatedPatients.length === 0 ? (
            ...
            paginatedPatients.map((patient) => {
              ...
              const hasAlerts =
                patient.injectionAlerts && patient.injectionAlerts.length > 0;
              return (
                <Card
                  key={patient.id}
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>
                          ID: {patient.id} | Personal #:{" "}
                          {patient.personalNumber} | Visit ID: {patient.visitId}
                        </CardDescription>
                        <CardDescription className="mt-1">
                          Clinic: {patient.clinic} | Visit Type:{" "}
                          {patient.visitType}
                        </CardDescription>
                        <div className="text-xs text-muted-foreground mt-1">
                          {patient.gender} • Age: {patient.age} •{" "}
                          {patient.employeeCategory} • {patient.phoneNumber}
                        </div>
                  ...
                  <CardContent className="flex justify-between items-center">
                    <div className="flex gap-2 flex-wrap">
                      <Button
                        ...
                        size="sm"
                        onClick={() => {
                          setSelectedPatientInjections(
                            injectionsByPatientId[patient.id] || {
        ...
        <Dialog open={historyOpen} onOpenChange={setHistoryOpen}>
          <DialogContent className="max-w-4xl max-h-[85vh] overflow-hidden">
            <DialogHeader>
              <DialogTitle>
                Injection History - {selectedPatientInjections?.patientName}
              </DialogTitle>
            </DialogHeader>
            <div className="overflow-y-auto pr-2" style={{ maxHeight: "65vh" }}>
              {selectedPatientInjections ? (
                (() => {
                  const allInjections =
                    selectedPatientInjections.injectionsHistory || [];
                  const grouped: Record<string, InjectionRecord[]> = {};
                  allInjections.forEach((v) => {
                    if (!grouped[v.date]) grouped[v.date] = [];
                    grouped[v.date].push(v);
                  });
                  const uniqueDates = Object.keys(grouped).sort((a, b) =>
                    b.localeCompare(a)
                  );
                  ...
                  if (currentDate) {
                    records = grouped[currentDate]
                      .slice()
                      .sort((a, b) => b.time.localeCompare(a.time));
                  }
                  const currentInjectionGroup = currentDate
                    ? [{ date: currentDate, records }]
                    : [];
                  return (
                    <>
                      {currentInjectionGroup.map(({ date, records }) => {
                        ...
                        return (
                          <div
                            key={date}
                            className="space-y-3 border rounded-lg p-4 mb-6"
                          >
                            {/* Visit-level header */}
                            <div className="flex items-start justify-between">
                              <div>
                                ...
                                <p className="text-sm text-gray-600">
                                  Personal Number:{" "}
                                  {selectedPatientInjections.personalNumber}
                                </p>
                                <p className="text-sm text-gray-600">
                                    ...
                                    <div className="flex items-center justify-between p-3">
                                      <div className="text-sm">
                                        <div className="font-medium">
                                          {rec.time} —{" "}
                                          {rec.injection.medication}{" "}
                                          {rec.injection.dosage && (
                                            <>({rec.injection.dosage})</>
                                          )}
                                        </div>
                                        <div className="text-xs text-gray-600 mt-1">
                                          Administered by {rec.recordedBy} via{" "}
                                          {rec.injection.route}
                                        </div>
                                        {rec.alerts.length > 0 && (
                                    ...
                                    {open && (
                                      <div className="p-3 pt-0 space-y-3">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                          {Object.entries(rec.injection).map(
                                            ([k, v]) =>
                                              k !== "comment" ? (
                                                <div
                                                  key={k}
                            ...
                            <PaginationContent>
                              ...
                              <PaginationItem>
                                <PaginationNext
                                  onClick={() =>
                                    setHistoryPage((p) =>
                                      Math.min(historyTotalPagesLocal, p + 1)
                                    )
                                  }
                                />
                              </PaginationItem>
        ...
        <Dialog open={addInjectionOpen} onOpenChange={setAddInjectionOpen}>
          <DialogContent className="max-w-2xl">
            ...
            <InjectionForm
              ...
              currentNurse={
                selectedPatientForInjection?.assignedNurse || "Current Nurse"
              }
            />
          </DialogContent>
        </Dialog>
