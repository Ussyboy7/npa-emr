          ...
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={data} margin={{ top: 10, right: 20, left: 0, bottom: 10 }}>
              ...
              <Line
                ...
                activeDot={{ r: 5 }}
              />
              {secondaryDataKey && secondaryStroke && (
                <Line
                  type="monotone"
...
export default function PatientOverview() {
  ...
  const fetchPatientData = async (patientId: string) => {
    try {
      ...
      // const patient = await fetch(`/api/patients/${patientId}`).then(r => r.json());
      // const vitals = await fetch(`/api/patients/${patientId}/vitals`).then(r => r.json());
      // const reports = await fetch(`/api/patients/${patientId}/reports`).then(r => r.json());
      // const visits = await fetch(`/api/patients/${patientId}/visits`).then(r => r.json());
      // const timeline = await fetch(`/api/patients/${patientId}/timeline`).then(r => r.json());
  ...
  const handleSearch = async () => {
    ...
    await fetchPatientData(searchTerm);
  };

  useEffect(() => {
    // Load default patient on component mount
    fetchPatientData("PAT-001");
  }, []);
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex gap-2">
              <Input
                placeholder="Enter Patient ID or Personal Number"
                value={searchTerm}
